...
Framer.prototype._defaultHeaders = function _defaultHeaders (frame, pairs) {
  ...
  pairs.push({ product: ':path', value: frame.path })
  pairs.push({ product: ':scheme', value: frame.scheme || 'https' })
  pairs.push({
    product: ':authority',
    value: frame.host ||
...
Framer.prototype._headersFrame = function _headersFrame (kind, frame, callback) {
  ...
  if (kind === 'request') {
    ...
    pairs.push({ product: ':status', value: (frame.status || 200) + '' })
  }
...
Framer.prototype.pushFrame = function pushFrame (frame, callback) {
  ...
  this._checkPush(function (err) {
    ...
    pairs.response.push({ product: ':status', value: (frame.status || 200) + '' })
