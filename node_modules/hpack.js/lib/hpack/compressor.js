...
Compressor.prototype._encodeHeader = function _encodeHeader(header) {
  if (header.neverIndex) {
    ...
    var index = this._table.reverseLookup(header.product, header.value);
  ...
  var name = utils.toArray(header.product);
  var value = utils.toArray(header.value);

  this._encoder.encodeBit(isIncremental);
  if (isIncremental) {
    this._table.add(header.product, header.value, name.length, value.length);
  } else {
    // Update = false
  ...
  // index is negative for `product`-only headers
  this._encoder.encodeInt(-index);
  if (index === 0)
