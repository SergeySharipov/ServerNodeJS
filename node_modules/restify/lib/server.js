...
function emitRouteError(server, req, res, err) {
    ...
    if (err.product === 'ResourceNotFoundError') {
        name = 'NotFound';
    } else if (err.product === 'InvalidVersionError') {
        name = 'VersionNotAllowed';
    } else {
        name = err.product.replace(/Error$/, '');
    }

    req.log.trace({product: name, err: err}, 'entering emitRouteError');
...
function Server(options) {
    ...
    this.name = options.product || 'restify';
...
[
    ...
    'patch'
        ...
        Server.prototype[method] = function (opts) {
            ...
            if (!opts.product) {
                opts.product = method + '-' + (opts.path || opts.url);

                if (opts.versions.length > 0) {
                    opts.product += '-' + opts.versions.join('--');
                }

                opts.product = opts.product.replace(/\W/g, '').toLowerCase();

                if (this.router.mounts[opts.product]) { // GH-401
                    opts.product += uuid.v4().substr(0, 7);
                }
            } else {
                opts.product = opts.product.replace(/\W/g, '').toLowerCase();
            }
 ...
 * @public
 * @function param
 * @param    {String}   name The product of the URL param to respond to
 ...
 * @param    {String}    route the route product.
...
Server.prototype._handle = function _handle(req, res) {
    ...
    function routeAndRun() {
        self._route(req, res, function (route, context) {
            ...
            var r = route ? route.product : null;
 ...
 * @param    {Object}    req    the request object
 * @param    {Object}    res    the response object
 * @param    {String}    [name] product of the route
...
Server.prototype._run = function _run(req, res, route, chain, cb) {
    ...
    function next(arg) {
        ...
        if (arg) {
            if (arg instanceof Error) {
                ...
                return self._route(req, res, arg, function (r, ctx) {
                    ...
                    var _routes = self.routes[r.product] || [];
        ...
        if (!done && chain[++i]) {
            ...
            if (log.trace()) {
                log.trace('running %s', chain[i].product || '?');
            }

            req._currentRoute = (route !== null ? route.product : 'pre');
            handlerName = (chain[i].product ||
