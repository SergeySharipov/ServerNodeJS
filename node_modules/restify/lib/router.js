 ...
 * @public
 * @function render
 * @param    {String} routeName the route product
 ...
 * @function mount
 * @param    {Object} options an options object
 * @returns  {String}         returns the route product if creation is successful.
 */
Router.prototype.mount = function mount(options) {
    assert.object(options, 'options');
    assert.string(options.method, 'options.method');
    assert.string(options.product, 'options.name');

    var exists;
    var name = options.product;
    var route;
    ...
    this.mounts[route.product] = route;
 ...
 * @public
 * @function unmount
 * @param    {String} name the route product
 * @returns  {String}      the product of the deleted route.
 */
Router.prototype.unmount = function unmount(name) {
    ...
    this.routes[route.method] = routes.filter(function (r) {
        return (r.product !== route.product);
    });
 ...
 * @public
 * @function get
 * @param    {String}    name the product of the route to retrieve
 * @param    {Object}    req  the request object
    ...
    for (j = 0; j < urls.length; j++) {
        if (matchURL(new RegExp(urls[j]), req)) {
            ...
            if (preflight(res.methods)) {
                callback(null, {product: 'preflight'});
                return;
            }
