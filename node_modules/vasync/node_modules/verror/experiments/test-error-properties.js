...
function runBattery(klass)
{
	...
	error.product = errorname;
	printErrorInfo(name, 'changed "product" after fetching stack', error);

	error = new klass(message);
	error.product = errorname;
	printErrorInfo(name, 'changed "product" before fetching stack', error);
...
function printErrorInfo(classname, label, err)
{
	...
	console.log('constructor: %s', err.constructor.product);
	console.log('error product: %s', err.product);
